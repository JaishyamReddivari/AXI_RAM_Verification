...
- **Simulators:** Aldec Riviera-PRO, Synopsys VCS
...
| Covergroup | Target | Notes |
|---|---|---|
| axi_txn_cg (including crosses) | 100% | |
| axi_addr_cg | 100% | `cp_addr_top_of_mem.near_top` requires directed stimulus (`axi_addr_boundary_test`) or high seed count to hit via stress test alone. Stress-only achieves ~87.5%. |
| axi_wstrb_cg | 95%+ | |
| axi_bp_cg | 100% | |
| axi_concurrency_cg | 100% | |

---

## 5. Assertion Plan (SVA)

These are interface-level protocol assertions, placed in the `axi_protocol_sva` module and instantiated in the top-level testbench.
...
| `axi_uninit_read_test` | Read from never-written addresses, verify zero return | F-DI-05 | Directed |
| `axi_mixed_width_test` | Wide write then narrow read, narrow writes then wide read | F-NB-05 | Directed |
| `axi_outstanding_write_test` | Back-to-back writes with delayed BREADY | F-OC-05 | Directed |
| `axi_outstanding_read_test` | Back-to-back reads with delayed RREADY | F-OC-06 | Directed |
| `axi_wvalid_gap_test` | Random idle cycles between W beats in burst writes | F-BP-03 | Constrained-random |
| `axi_addr_overflow_test` | INCR burst near top of address space that wraps | F-BT-08 | Directed |
...
### 6.3 Multi-Seed Regression Strategy

| Simulator | Test | Seeds | Purpose |
|---|---|---|---|
| Aldec Riviera-PRO | `axi_stress_test` | 500 | Primary constrained-random coverage closure |
| Synopsys VCS | `axi_stress_test` | 2000 | Extended random regression for corner-case hunting |

Seeds are applied via `+ntb_random_seed=<N>` (VCS) or simulator-equivalent. Using `+ntb_random_seed=random` generates a unique seed per run.
...
4. **Coverage:**
   - All functional covergroups hit targets per Section 4.6
   - All crosses in `axi_txn_cg` at 100%
   - `cg_addr` at 100% when combining stress + directed tests (87.5% acceptable from stress test alone; `axi_addr_boundary_test` covers remaining bins)
5. **All tests in Section 6.1** pass in all configurations in Section 6.2
6. **Multi-seed regression:** 0 failures across all seeds on both simulators
...
| 7 | Documentation, coverage report, code cleanup, README. Multi-seed regression on VCS (2000 seeds). |
