...
| F-WS-02 | All strobes off | Write with WSTRB = 4'b0000 | No memory update | P1 |
| F-WS-03 | Single byte strobe | Write with WSTRB = 4'b0001, 4'b0010, etc. | Only targeted byte changes | P0 |
| F-WS-04 | Strobe in burst | Different WSTRB per beat in a burst | Each beat applies its own strobe correctly | P1 |

### 3.5 ID Signal Correctness (F-ID)
...
| F-PC-06 | RDATA stable | RDATA must not change while RVALID=1 and RREADY=0 | SVA assertion | P0 |
| F-PC-07 | BRESP stable | BRESP must not change while BVALID=1 and BREADY=0 | SVA assertion | P0 |
| F-PC-08 | Reset behavior | Assert rst, observe all outputs | All VALID signals deasserted after reset | P0 |

### 3.8 Backpressure and Timing (F-BP)
...
| `axi_base_test` | Reset-only smoke test, confirms compile and reset behavior | F-PC-08 | Directed |
| `axi_single_rw_test` | N single write-readback pairs to sequential addresses | F-DI-01, F-DI-03, F-ID-01, F-ID-02, F-RC-01, F-RC-02 | Directed |
| `axi_burst_test` | Randomized INCR bursts (len 1-15), write+readback | F-BT-01, F-RC-03 | Constrained-random |
| `axi_fixed_burst_test` | FIXED burst writes, verify last-beat semantics | F-BT-03, F-BT-04 | Directed |
| `axi_narrow_burst_test` | AXSIZE=0 and AXSIZE=1 on 32-bit bus | F-NB-01 through F-NB-06 | Constrained-random |
| `axi_strobe_test` | Partial WSTRB patterns, verify byte-level masking | F-WS-01 through F-WS-04 | Directed + random |
| `axi_backpressure_test` | Random BREADY/RREADY delays during all transaction types | F-BP-01 through F-BP-06 | Constrained-random |
| `axi_stress_test` | 500+ random transactions, all burst types, random backpressure | All F-DI, F-BT, F-NB, F-BP | Constrained-random |
| `axi_id_test` | Sweep ID values across full range | F-ID-03, F-ID-04 | Directed |
| `axi_max_burst_test` | len=255 INCR burst | F-BT-02 | Directed |
| `axi_pipeline_test` | Repeat key tests with PIPELINE_OUTPUT=1 | F-PO-01 through F-PO-03 | Regression |
| `axi_addr_boundary_test` | Access addr 0x0000, 0xFFFC, near-boundary regions | F-DI-04 | Directed |
