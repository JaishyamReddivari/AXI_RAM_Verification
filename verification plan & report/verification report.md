...
All 14 protocol assertions from the verification plan passed across all test runs on both simulators with **zero failures**.

| SVA ID | Property | Implementation | Result |
|---|---|---|---|
| SVA-01 | VALID-before-READY stability (AW, W, AR, B, R) | 5 dedicated properties (`p_awvalid_stable`, etc.) | **PASS** |
| SVA-02 | Payload stable while waiting (AW, W, AR, B, R) | 5 dedicated properties (`p_aw_stable`, etc.) | **PASS** |
| SVA-03 | WLAST at correct beat (AWLEN+1) | `always_ff` block with beat counter + AW length queue | **PASS** |
| SVA-04 | RLAST at correct beat (ARLEN+1) | `always_ff` block with beat counter + AR length queue | **PASS** |
| SVA-05 | BID matches AWID | `always_ff` block with AWID queue, checked on B handshake | **PASS** |
| SVA-06 | RID matches ARID (all beats) | `always_ff` block with ARID queue, checked on R handshake | **PASS** |
| SVA-07 | No BVALID before WLAST accepted | Property `p_no_b_before_wlast` with `wlast_done` tracker | **PASS** |
| SVA-08 | Reset clears DUT outputs | Property `p_reset_clears` on `$fell(rst)` | **PASS** |
| SVA-09 | BRESP = OKAY (2'b00) | Property `p_bresp_okay` | **PASS** |
| SVA-10 | RRESP = OKAY (2'b00) | Property `p_rresp_okay` | **PASS** |
| SVA-11 | BID stable while BVALID && !BREADY | Covered by SVA-02 (`p_b_stable` checks `$stable(bid)`) | **PASS** |
| SVA-12 | RID stable while RVALID && !RREADY | Covered by SVA-02 (`p_r_stable` checks `$stable(rid)`) | **PASS** |
| SVA-13 | RLAST stable while RVALID && !RREADY | Covered by SVA-02 (`p_r_stable` checks `$stable(rlast)`) | **PASS** |
| SVA-14 | No RVALID before AR handshake | Property `p_no_rvalid_without_ar` with `ar_pending` counter | **PASS** |

**Note on SVA-11/12/13:** The verification plan defines these as separate assertions for BID stability, RID stability, and RLAST stability. In the implementation, these checks are subsumed by SVA-02's payload stability properties — `p_b_stable` asserts `$stable(bid) && $stable(bresp)`, and `p_r_stable` asserts `$stable(rid) && $stable(rdata) && $stable(rresp) && $stable(rlast)`. This provides equivalent or stronger coverage since the entire payload (not just the individual signal) is checked for stability. The SVA code numbers from SVA-10 directly to SVA-14 for this reason; SVA-11 through SVA-13 are logically present but not given separate assertion labels.
...
|---|---|---|
| `axi_ram.v` | `design/` | DUT — AXI4 slave RAM (unmodified, from alexforencich/verilog-axi) |
| `axi_if.sv` | `verification/` | AXI4 interface definition |
| `axi_ram_pkg.sv` | `verification/` | UVM package: seq_item, driver, monitor, scoreboard, coverage, agent, env, 16 sequences, 22 tests |
| `axi_protocol_sva.sv` | `verification/` | 14 SVA protocol assertions (SVA-11/12/13 covered within SVA-02; see Section 1.3) |
| `tb_top.sv` | `verification/` | Top-level testbench: DUT instantiation, clock/reset, config, UVM launch |

**EDA Playground note:** Due to the two-file limitation on EDA Playground, the SVA module is appended to `design.sv` (after the DUT) and the interface, package, and testbench module are combined into `testbench.sv`. In the repository, each file is separate as listed above.
...
- On EDA Playground, the SVA module is appended to `design.sv` and all testbench files are combined into `testbench.sv` due to the two-file limitation; in the repository, each file is separate (see Section 5)
...
- `cg_addr` achieves 87.50% under `axi_stress_test` alone due to the `cp_addr_top_of_mem.near_top` bin requiring addresses in a very narrow range (>= 0xFFF0); this bin is covered by the directed `axi_addr_boundary_test`
